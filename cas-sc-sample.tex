...
Neurobiological phenotypes are described at complementary levels by brain structure and function, genetic susceptibility, clinical measurements, and behavioral or environmental factors~\cite{ref_jack2018niaaa,ref_lane2018ad,ref_scheltens2021lancet}. Multimodal learning can combine evidence absent from any single source through representation and fusion~\cite{ref_zhang2011multimodal,ref_suk2014deep,ref_venugopalan2021,ref_liu2018survey,baltrusaitis2019multimodal,ngiam2011multimodal,ref_mag}. Its value in neurodevelopmental and neurodegenerative prediction, however, depends on using the evidence actually available for each subject rather than assuming a complete input set~\cite{ref_esteva2019guide}.
...
We evaluate CERD on the Adolescent Brain Cognitive Development (ABCD) Study and the Alzheimer's Disease Neuroimaging Initiative (ADNI)~\cite{garavan2018abcd,ref_adni}. ABCD distinguishes low current symptom burden, subthreshold current ADHD, and current full ADHD through disjoint K-SADS symptom intervals and clinical conditions. ADNI distinguishes cognitively normal, mild cognitive impairment, and Alzheimer's disease. Each study uses four modality groups and the incomplete-input protocol in Section~\ref{sec:datasets}. We report accuracy, macro-F1, and macro-AUROC. The principal contributions are:

\begin{itemize}
    \item We propose CERD, a unified incomplete multimodal framework combining recovery, provenance, sparse specialization, and reliability-based prediction.
    \item Target-specific generators infer an unavailable latent modality from the same subject's observed subset, rather than from the missingness pattern alone.
    \item Observed and generated provenance conditions self-attention and top-\(k\) expert routing, making specialization sensitive to evidence origin.
    \item Global, modality-centered, and pair-centered predictions form an exact normalized probability decomposition. Three-seed studies test its components and source relevance by strict removal.
\end{itemize}
...
Subset-native models retain only measured information, whereas conditional recovery restores a source-indexed layout that supports downstream interactions. For heterogeneous sources, recovery in a shared task space also avoids requiring imaging, genetic, clinical, and behavioral variables to share an observation space. CERD therefore reconstructs target-specific latent tokens, preserves their generated origin, and estimates their predictive trust instead of equating marginal reconstruction similarity with subject-specific utility.
...
CERD connects functions usually treated separately: subject-conditioned generation supplies missing latent evidence, provenance conditions interaction and routing, and reliability controls decision mass. Thus reconstruction does not erase source status, and the final mixture remains part of inference.
...
The attention and expert stages have complementary roles. Global self-attention communicates evidence across every modality position, whereas sparse feed-forward experts specialize the transformation applied to each resulting token. The router therefore adapts computation to both the subject and the observed--generated composition without creating a separate network for every missingness pattern. With \(M=4\), the shared sequence contains 64 tokens on ADNI and 32 on ABCD. ADNI activates the top four of 16 experts per token, whereas ABCD activates the top two of four. The MoE remains a shared representation backbone, and all subsequent diagnostic branches reuse its pooled modality features.
...
\noindent\textbf{ABCD.}
The ABCD Study characterizes adolescent brain, cognitive, behavioral, and health development~\cite{garavan2018abcd}. We derive an ordered cross-sectional current-severity endpoint from the baseline parent K-SADS assessment. Let \(s\) be the larger positive-symptom count across the nine-item inattention and hyperactivity/impulsivity domains. Class~0 requires \(s\in[0,2]\), no current, past, partial-remission, or unspecified ADHD status, and no current functional impairment. Class~1 requires \(s\in[3,5]\), no current full ADHD, and a positive K-SADS onset/cross-setting condition. Class~2 requires \(s\in[6,9]\), current full ADHD, current impairment, and the onset/cross-setting condition. The non-overlapping symptom intervals make the middle group a current subthreshold state rather than a diagnosis--symptom discordance group; the endpoint describes severity, not longitudinal progression. The \(3{,}000\)-participant cohort contains \(1{,}733\), \(404\), and \(863\) subjects in classes 0, 1, and 2. Family-disjoint training, validation, and test splits contain \(2{,}134\), \(438\), and \(428\) participants, with respective class counts \((1{,}222,277,635)\), \((262,66,110)\), and \((249,61,118)\).

ABCD predictors form four model modalities. Imaging (785 variables) concatenates rs-fMRI network connectivity and regional variance, regional beta contrasts from SST, n-back, and MID task-fMRI, regional T1-weighted gray-matter intensity, and DTI-derived FA. Genetics comprises 1,172 directly encoded SNP dosages retained after training-set missingness, minor-allele-frequency, and LD pruning within 14 prespecified gene windows; no PCA, PRS, or ancestry components are used. The cognition and health group contains 90 NIH Toolbox, WISC, Little Man, RAVLT, sleep, and physical-activity variables. The behavior and environment group begins with 221 non-ADHD behavioral, family, neighborhood, socioeconomic, and address-linked exposure variables, of which 219 survive training-only screening. Target-defining K-SADS variables and CBCL ADHD/attention scores are excluded from every predictor group.
...
\midrule
Training & 409/1480 (27.64) & 320/2134 (15.00) \\
Validation & 91/318 (28.62) & 66/438 (15.07) \\
Testing & 99/318 (31.13) & 64/428 (14.95) \\
\bottomrule
\end{tabular*}
\end{minipage}
\end{center}
...
CERD uses hidden dimension \(D=128\) and one four-head fusion layer. ADNI uses \(P=16\) tokens per modality with 16 experts and top-4 routing; ABCD uses \(P=8\), four experts, and top-2 routing. Each target generator contains two cross-attention layers. We use \((\lambda_b,\lambda_g)=(0.1,0.01)\), \((\lambda_d,\lambda_{\mathrm{sd}},\lambda_{\mathrm{mf}})=(0.1,0.15,0.1)\), \(T=2\), and \(\epsilon=10^{-3}\). The reconstruction coefficient is 1.0 on ADNI and 0.5 on ABCD. ABCD models complete 100 epochs with AdamW and a learning rate of \(1.0\!\times\!10^{-4}\); the class weight for class \(c\) is proportional to \(n_c^{-0.75}\). Every method uses the same rule. Checkpoints are selected by validation macro-F1 and the test set is evaluated once after selection. Results are arithmetic mean \(\pm\) sample standard deviation over seeds 0/1/2 for ADNI and 31/32/33 for ABCD; probabilities are never ensembled across seeds.

\label{sec:evaluation}

\noindent\textbf{Evaluation metrics.}
We report accuracy, macro-F1, and class-averaged AUROC. Accuracy measures overall decisions, macro-F1 weights every endpoint class equally, and AUROC summarizes class ranking independently of one operating point. The experiments ask whether CERD improves prediction under heterogeneous missingness, whether each stage contributes, whether performance is retained for incomplete participants, and whether model-output relevance agrees with controlled modality interventions. ABCD comparisons use the same family-disjoint split and fixed masks for every method, with training-split preprocessing throughout.
...
We compare CERD with six representative methods for incomplete multimodal learning. Flex-MoE combines a missing-modality bank with sparse routing~\cite{ref_flexmoe}. I$^2$MoE models unique, redundant, and synergistic multimodal interactions~\cite{xin2025i2moe}, whereas MoE++-corrected introduces learned, zero, copy, and constant experts~\cite{jin2025moepp}. AnyMod uses modality queries and a task anchor to accommodate arbitrary input combinations~\cite{feng2024anymod}. AGDiC and ACADiff are evaluated through task-matched feature-space implementations because their original formulations operate on different neuroimaging representations~\cite{li2025agdic,zhou2026acadiff}. All methods use the same participant splits, modality groups, missingness patterns, and evaluation metrics.
...
AGDiC~\cite{li2025agdic}
& 58.60$\pm$3.36 & 56.54$\pm$2.38 & 75.10$\pm$1.36 \\
ACADiff~\cite{zhou2026acadiff}
...
\begin{table}[t]
\centering
\caption{Comparison on family-disjoint, three-class ABCD current-severity classification with approximately 15\% incomplete participants (\%). AUC denotes macro-AUROC; best means are boldfaced.}
...
\midrule
Flex-MoE
& 63.79$\pm$2.60 & 51.44$\pm$2.28 & 74.69$\pm$1.30 \\
I$^2$MoE
& 63.24$\pm$0.82 & 49.46$\pm$1.27 & 74.60$\pm$0.92 \\
MoE++-corrected
& 64.64$\pm$2.22 & 52.06$\pm$4.35 & 74.44$\pm$1.81 \\
AnyMod
& 62.15$\pm$2.00 & 49.09$\pm$0.61 & 69.17$\pm$4.68 \\
AGDiC
& 62.69$\pm$3.04 & 48.83$\pm$0.84 & 71.71$\pm$2.66 \\
ACADiff
& 63.16$\pm$1.18 & 49.05$\pm$1.56 & 70.90$\pm$0.78 \\
\textbf{CERD (Ours)}
& \textbf{66.82$\pm$2.14} & \textbf{55.18$\pm$1.16} & \textbf{77.90$\pm$0.55} \\
\bottomrule
\end{tabular*}
\end{table}

On ABCD, CERD gives the highest accuracy, macro-F1, and macro-AUROC, exceeding the strongest competing means by 2.18, 3.12, and 3.20 points, respectively. The larger macro-F1 margin indicates that the gain is not explained by the low-symptom majority alone; the simultaneous AUC increase shows stronger one-vs-rest ranking across the ordered severity groups.
...
ABCD & Complete & 364 & 67.22$\pm$2.22 & 54.83$\pm$2.11 & 77.51$\pm$1.33 \\
 & Incomplete & 64 & 64.58$\pm$4.77 & 55.06$\pm$4.59 & 80.13$\pm$3.81 \\
\bottomrule
\end{tabular*}
\end{table}

Table~\ref{tab:availability_strata} shows that CERD remains operational for incomplete participants in both cohorts. On ABCD, the complete and incomplete strata have similar mean metrics, while the 64-subject incomplete stratum has much larger seed variability. Because these strata contain different participants, they describe observed deployment conditions rather than the causal effect of removing a source; the matched within-participant interventions in Section~\ref{sec:decision_analysis} address the latter question.

\subsection{Ablation Study}
\label{sec:ablation_results}

\noindent\textbf{Compared variants.}
Each control starts from the dataset's full configuration and removes one named mechanism. Conditional completion is bypassed; provenance embeddings are suppressed; expert diversity is removed by setting \(E=k=1\); or multigranular decomposition is reduced to the global head with its specialist objectives removed. For ABCD, a capacity-aligned Dense FFN replaces sparse expert routing, and the reliability control fixes every usable \(r_n^m\) to one while retaining entropy confidence and learned branch priors. Every variant is independently trained and validation-selected with the same three seeds as its main result. The two datasets are reported separately because their missingness mechanisms and endpoints differ.

\begin{table}[t]
\centering
\caption{Component analysis on the full ADNI test cohort (\%, mean $\pm$ SD). Best means are boldfaced.}
...
\midrule
w/o Conditional Completion & 63.73$\pm$0.79 & 62.82$\pm$1.58 & 79.48$\pm$0.41 \\
w/o Provenance Embeddings & 63.52$\pm$1.57 & 63.41$\pm$1.95 & 80.50$\pm$0.70 \\
w/o Expert Diversity ($E=1$) & 64.05$\pm$0.65 & 63.70$\pm$0.59 & \textbf{81.13$\pm$1.02} \\
w/o Multigranular Decomposition & 62.47$\pm$0.79 & 62.36$\pm$1.00 & 79.72$\pm$1.32 \\
w/o Reliability-aware Weights & 64.05$\pm$1.49 & 63.88$\pm$0.21 & 80.95$\pm$0.19 \\
\textbf{Full CERD} & \textbf{65.72$\pm$1.09} & \textbf{64.56$\pm$2.09} & 81.07$\pm$0.55 \\
\bottomrule
\end{tabular*}
\end{table}
...
\begin{table}[t]
\centering
\caption{Component analysis on the full ABCD test cohort (\%, mean $\pm$ SD). Best means are boldfaced.}
...
\textbf{Configuration} & \textbf{Acc.} & \textbf{Macro-F1} & \textbf{AUC} \\
\midrule
w/o Conditional Completion & 64.41$\pm$2.77 & 53.34$\pm$2.43 & 75.37$\pm$3.08 \\
w/o Provenance Embeddings & 65.03$\pm$2.31 & 53.07$\pm$1.09 & 77.14$\pm$1.75 \\
w/o Expert Diversity ($E=1$) & 64.95$\pm$1.76 & 52.82$\pm$2.22 & 77.79$\pm$0.47 \\
Dense FFN & 66.43$\pm$2.35 & 53.81$\pm$0.77 & 77.30$\pm$0.79 \\
w/o Multigranular Decomposition & 63.63$\pm$1.99 & 53.73$\pm$0.65 & 76.73$\pm$0.63 \\
w/o Modality Reliability & 66.04$\pm$3.28 & 53.91$\pm$1.88 & 77.44$\pm$1.38 \\
\textbf{Full CERD} & \textbf{66.82$\pm$2.14} & \textbf{55.18$\pm$1.16} & \textbf{77.90$\pm$0.55} \\
\bottomrule
\end{tabular*}
\end{table}

\noindent\textbf{Ablation results.}
Tables~\ref{tab:adni_ablation} and~\ref{tab:abcd_ablation} identify complementary roles rather than interchangeable additions. Reducing CERD to the global head causes the largest accuracy loss on both datasets: 3.25 points on ADNI and 3.19 on ABCD. The source-centered and pair-centered readouts therefore recover decision structure that one pooled representation does not preserve. Completion has the clearest availability-specific effect. Among the 64 incomplete ABCD participants, removing it lowers accuracy, macro-F1, and AUC from 64.58/55.06/80.13 to 58.33/45.87/75.08, linking reconstructed tokens directly to decisions under missing input rather than to reconstruction similarity alone. Removing provenance or modality reliability lowers overall ABCD macro-F1 by 2.10 and 1.27 points, respectively.

The expert controls separate sparse specialization from the remaining CERD mechanisms. Relative to \(E=1\), the full mixture gains 1.67/0.86 Accuracy/macro-F1 points on ADNI and 1.87/2.36 points on ABCD. On ABCD, full CERD also exceeds the capacity-aligned Dense FFN on all three metrics. The difference is most visible when a source is absent: CERD leads Dense by 1.56 Accuracy, 4.13 macro-F1, and 2.66 AUC points in the incomplete stratum. Sparse routing thus contributes through availability-dependent specialization, not merely additional parameters.
...
\midrule
ABCD & Imaging & 19.21$\pm$9.62 & 14.04$\pm$23.72 & 9.98$\pm$18.99 \\
 & Genetics & 30.90$\pm$1.04 & 57.05$\pm$14.71 & 28.75$\pm$2.24 \\
 & Cog./health & 23.88$\pm$6.43 & 12.79$\pm$7.73 & 5.95$\pm$2.75 \\
 & Behav./env. & 26.01$\pm$3.09 & 16.12$\pm$2.98 & 8.42$\pm$2.22 \\
\bottomrule
\end{tabular*}
\end{table}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.94\textwidth]{cerd_two_dataset_modality_drop_severity3_e4k2_v2.pdf}
\caption{Normalized prediction-change and accuracy-drop shares under strict modality removal (three-seed means). I/G/C/B denote MRI/genetics/clinical/biospecimen on ADNI and imaging/genetics/cognition--health/behavior--environment on ABCD.}
\label{fig:two_dataset_modality_drop}
\end{figure*}

Table~\ref{tab:modality_contribution} and Fig.~\ref{fig:two_dataset_modality_drop} show that label-free prediction changes recover the main ordering of label-dependent performance effects. Pearson correlations between normalized change and accuracy-drop shares are \(0.979\) on ADNI and \(0.825\) on ABCD; both datasets have Spearman \(\rho=0.8\). ADNI concentrates functional dependence in structural MRI and biospecimens, consistent with its neurodegenerative endpoint. In ABCD, direct QC/LD-pruned SNP dosage has the largest prediction-change share and causes a 28.75-point mean accuracy decrease when strictly removed. Imaging, cognition--health, and behavior--environment retain secondary contributions, but none substitutes for the genetic signal. This ordering is obtained from interventions on the frozen multimodal classifier rather than from marginal feature distributions or an external PRS.

\section{Discussion}
\label{sec:discussion}

\noindent\textbf{Coupling completion with controlled trust.}
CERD leads all three main metrics on ADNI and ABCD. The incomplete-stratum control shows where reconstruction matters: conditional tokens recover decisions that are unavailable to an observed-subset-only model, while provenance and modality reliability regulate how that recovered evidence enters the branch mixture. This coupling prevents completion from becoming an isolated representation objective: its output is evaluated according to both origin and task-dependent trust before contributing to the final probability.

\noindent\textbf{Sparse specialization of completed evidence.}
Provenance exposes content, modality position, and evidence origin before expert routing. The one-expert controls lower accuracy and macro-F1 in both cohorts, and the capacity-aligned ABCD Dense FFN loses most clearly on incomplete participants. Since the Dense control retains the encoders, completion, token layout, and readouts, this gap isolates content-dependent expert transformations under changing source availability rather than capacity alone.

\noindent\textbf{Why multigranular decomposition matters.}
Reducing CERD to its global head produces the largest accuracy loss in both datasets. Since each \(\mathbf f_n^m\) already contains cross-modal context, modality and pair anchors expose source-centered agreement--discrepancy views beyond one pooled representation without treating them as independent unimodal predictions. Their normalized mixture also keeps these views auditable in the same probability that is used for classification.

\noindent\textbf{Prediction consistency.}
ADNI depends most on structural MRI and biospecimens, whereas the severity-defined ABCD decision is most sensitive to direct SNP dosage. Prediction-change shares track independently measured accuracy-drop shares in both cohorts, connecting the model's label-free response to a source intervention with its measured predictive dependence. The cohort-specific ordering is not imposed by the fusion rule; it emerges from the same architecture trained on distinct endpoints and source groups.

\section{Conclusion}

CERD unifies conditional token recovery, provenance-aware expert fusion, and reliability-based evidence decomposition. It ranks first for all three reported metrics on ADNI and family-disjoint ABCD severity classification. Matched controls isolate completion, sparse specialization, and multigranular readout, while strict-removal agreement connects output sensitivity to measured source dependence.
