...
We evaluate CERD on the Adolescent Brain Cognitive Development (ABCD) Study and the Alzheimer's Disease Neuroimaging Initiative (ADNI)~\cite{garavan2018abcd,ref_adni}. ABCD distinguishes low-symptom children, symptom-positive children without a recorded diagnosis, and current full ADHD. ADNI distinguishes cognitively normal, mild cognitive impairment, and Alzheimer's disease. Each study uses four modality groups and the incomplete-input protocol in Section~\ref{sec:datasets}. We report accuracy, macro-F1, and macro-AUROC. The principal contributions are:
...
\noindent\textbf{ABCD.}
The ABCD Study characterizes adolescent brain, cognitive, behavioral, and health development~\cite{garavan2018abcd}. We derive a three-class current-status endpoint from the baseline parent K-SADS assessment. Class~0 is a low-symptom comparator: current, past, partial-remission, and unspecified ADHD status are all non-positive, with at most three current symptoms in each nine-item domain. Class~1 is symptom-positive without a recorded ADHD status: the same four status fields are non-positive, but at least one current domain contains four or more symptoms. Class~2 requires current full ADHD together with at least six current symptoms in one domain. Class~1 therefore denotes a distinct symptom--diagnosis discordance state rather than mild diagnosed ADHD. The \(3{,}000\)-participant cohort contains \(1{,}677\), \(460\), and \(863\) subjects in classes 0, 1, and 2. Family-disjoint training, validation, and test splits contain \(2{,}141\), \(430\), and \(429\) participants, with respective class counts \((1{,}195,331,615)\), \((244,67,119)\), and \((238,62,129)\).

ABCD predictors form four model modalities. Imaging (785 variables) concatenates rs-fMRI network connectivity and regional variance, regional beta contrasts from SST, n-back, and MID task-fMRI, regional T1 gray-matter volume, and DTI-derived FA. Genetics comprises 1,184 directly encoded SNP dosages retained after training-set missingness, minor-allele-frequency, and LD pruning within 14 prespecified gene windows; no PCA, PRS, or ancestry components are used. The cognition and health group contains 90 NIH Toolbox, WISC, Little Man, RAVLT, sleep, and physical-activity variables. The behavior and environment group begins with 221 non-ADHD behavioral, family, neighborhood, socioeconomic, and address-linked exposure variables, of which 219 survive training-only screening. Target-defining K-SADS variables and CBCL ADHD/attention scores are excluded from every predictor group.
...
\midrule
Training & 409/1480 (27.64) & 321/2141 (14.99) \\
Validation & 91/318 (28.62) & 64/430 (14.88) \\
Testing & 99/318 (31.13) & 64/429 (14.92) \\
\bottomrule
\end{tabular*}
\end{minipage}
\end{center}
...
\begin{table}[t]
\centering
\caption{Comparison on family-disjoint, three-class ABCD current-status classification with approximately 15\% incomplete participants (\%). AUC denotes macro-AUROC; best means are boldfaced.}
...
\midrule
Flex-MoE
& 63.87$\pm$3.03 & 51.27$\pm$3.15 & 74.09$\pm$2.93 \\
I$^2$MoE
& 63.25$\pm$3.10 & 53.90$\pm$1.35 & 74.54$\pm$1.01 \\
MoE++-corrected
& 64.80$\pm$0.62 & 53.58$\pm$1.08 & 75.11$\pm$0.76 \\
AnyMod
& 61.69$\pm$4.00 & 53.57$\pm$2.25 & 74.95$\pm$1.29 \\
AGDiC
& 63.79$\pm$0.75 & 52.52$\pm$1.72 & \textbf{75.16$\pm$0.53} \\
ACADiff
& 63.17$\pm$2.69 & 51.68$\pm$2.95 & 73.05$\pm$1.03 \\
\textbf{CERD (Ours)}
& \textbf{67.37$\pm$1.07} & \textbf{54.38$\pm$0.81} & 74.75$\pm$0.92 \\
\bottomrule
\end{tabular*}
\end{table}

On ABCD, CERD gives the highest accuracy and macro-F1, exceeding the strongest competing mean by 2.57 and 0.48 points, respectively. The larger accuracy margin shows that the completed evidence improves the overall decision boundary, whereas the narrower macro-F1 margin reflects the difficulty of the smaller symptom-positive/no-status class. AGDiC gives the highest AUROC by 0.41 points; CERD therefore improves hard three-way decisions without claiming uniformly superior class ranking.
...
ABCD & Complete & 365 & 67.49$\pm$1.14 & 55.33$\pm$0.58 & 75.64$\pm$0.84 \\
 & Incomplete & 64 & 66.67$\pm$0.90 & 46.33$\pm$3.87 & 72.45$\pm$2.86 \\
\bottomrule
\end{tabular*}
\end{table}

Table~\ref{tab:availability_strata} shows that CERD remains operational for incomplete participants in both cohorts. On ABCD, accuracy changes little across availability strata, but macro-F1 is lower for the 64 incomplete participants. This separation indicates that missing inputs disproportionately affect the two smaller clinical groups even when the majority-class decision rate is retained. Because the strata contain different participants, they describe deployment conditions rather than the causal effect of removing a source; the matched within-participant interventions in Section~\ref{sec:decision_analysis} address the latter question.

\subsection{Ablation Study}
\label{sec:ablation_results}

\noindent\textbf{Compared variants.}
Each control starts from the dataset's full configuration and removes one named mechanism. Conditional completion is bypassed; provenance embeddings are suppressed; expert diversity is removed by setting \(E=k=1\); multigranular decomposition is reduced to the global head with its specialist objectives removed; or adaptive branch scores are replaced by a uniform simplex. Every variant is independently trained and validation-selected with the same three seeds as its main result. The two datasets are reported separately because their missingness mechanisms and endpoints differ.
...
\textbf{Configuration} & \textbf{Acc.} & \textbf{Macro-F1} & \textbf{AUC} \\
\midrule
w/o Conditional Completion & 66.74$\pm$3.46 & \textbf{54.46$\pm$1.29} & 73.64$\pm$0.72 \\
w/o Provenance Embeddings & 65.97$\pm$1.68 & 54.19$\pm$0.23 & 74.49$\pm$0.68 \\
w/o Expert Diversity ($E=1$) & 63.71$\pm$0.71 & 52.14$\pm$0.61 & 73.73$\pm$0.69 \\
w/o Multigranular Decomposition & 61.07$\pm$2.84 & 50.14$\pm$3.50 & 71.41$\pm$3.83 \\
w/o Reliability-aware Weights & 66.20$\pm$3.85 & 53.94$\pm$1.40 & 73.66$\pm$0.77 \\
\textbf{Full CERD} & \textbf{67.37$\pm$1.07} & 54.38$\pm$0.81 & \textbf{74.75$\pm$0.92} \\
\bottomrule
\end{tabular*}
\end{table}

\noindent\textbf{Ablation results.}
Tables~\ref{tab:adni_ablation} and~\ref{tab:abcd_ablation} identify multigranular decomposition as the largest shared effect: reducing CERD to its global head lowers accuracy by 3.25 points on ADNI and 6.30 points on ABCD, with corresponding macro-F1 losses of 2.20 and 4.24 points. Expert diversity is also consequential. Relative to \(E=1\), the full sparse mixture improves accuracy/macro-F1 by 1.67/0.86 points on ADNI and 3.66/2.24 points on ABCD; the small ADNI AUC reversal indicates that the gain primarily changes the operating decision rather than ranking. On ABCD, completion raises accuracy and AUC by 0.63 and 1.11 points, while its 0.08-point macro-F1 difference is negligible. Provenance and learned weights provide smaller but consistent gains in ABCD accuracy.

An aligned Dense-FFN capacity control is reported separately from component removal. On all ABCD test participants it obtains 66.74/55.00/75.26 versus CERD's 67.37/54.38/74.75, showing a metric trade-off. On the 64 incomplete participants, CERD exceeds Dense by 2.61 accuracy, 2.17 macro-F1, and 0.65 AUC points. ADNI also shows mixed Dense effects (64.99/65.27/81.59 overall). Together with the \(E=1\) control, these results locate the clearest sparse-expert benefit in diverse routing under incomplete ABCD inputs.
...
\midrule
ABCD & Imaging & 15.02$\pm$5.02 & 12.06$\pm$10.57 & 5.02$\pm$3.58 \\
 & Genetics & 22.07$\pm$10.83 & 28.10$\pm$25.47 & 15.34$\pm$17.73 \\
 & Cog./health & 30.91$\pm$4.33 & 27.63$\pm$9.26 & 11.78$\pm$0.99 \\
 & Behav./env. & 32.01$\pm$3.73 & 32.20$\pm$10.40 & 13.79$\pm$1.14 \\
\bottomrule
\end{tabular*}
\end{table}
...
Table~\ref{tab:modality_contribution} and Fig.~\ref{fig:two_dataset_modality_drop} show that label-free prediction changes recover the main ordering of label-dependent performance effects. Pearson correlations between normalized change and accuracy-drop shares are \(0.979\) on ADNI and \(0.874\) on ABCD; both datasets have Spearman \(\rho=0.8\). ADNI concentrates functional dependence in structural MRI and biospecimens. On ABCD, behavior/environment produces the largest and most stable accuracy decrease, cognition/health changes a similar fraction of decisions, and SNP removal has a large but seed-variable effect. Imaging remains contributory but is least disruptive in the joint model. This profile is consistent with an ADHD endpoint expressed through behavioral and cognitive function while retaining distributed genetic and neuroimaging information.

\section{Discussion}
\label{sec:discussion}

\noindent\textbf{Coupling completion with controlled trust.}
CERD leads all three ADNI metrics and gives the highest accuracy and macro-F1 on the three-class ABCD task. Removing conditional completion lowers every ADNI metric and reduces ABCD accuracy and AUROC. A cumulative Dense/global control further isolates the missing-input effect: adding completion raises incomplete-ABCD accuracy and macro-F1 by 2.08 and 2.30 points.

\noindent\textbf{Sparse specialization of completed evidence.}
Provenance precedes routing, so experts specialize using semantic content, modality position, and evidence origin. Replacing the expert pool by one expert lowers accuracy and macro-F1 on both datasets, with larger reductions of 3.66 and 2.24 points on ABCD. The aligned Dense control further shows that CERD's clearest advantage occurs in the incomplete ABCD stratum, where routing must accommodate heterogeneous observed--generated token layouts.

\noindent\textbf{Why multigranular decomposition matters.}
Reducing CERD to its global head produces the largest component loss in both cohorts, including 3.25 and 6.30 accuracy points on ADNI and ABCD. Because each \(\mathbf f_n^m\) already contains cross-modal context, modality and pair anchors expose source-centered and agreement--discrepancy views of a shared state, adding decision structure beyond one pooled representation.

\noindent\textbf{Prediction consistency.}
ADNI depends most on structural MRI and biospecimens, whereas the ABCD decision is most frequently changed by behavior and environment, followed by cognition and health. Prediction-change shares track independently measured accuracy-drop shares in both cohorts, connecting label-free sensitivity of the frozen classifier to observed predictive dependence.

\section{Conclusion}

CERD combines conditional token recovery, sparse provenance-aware expert fusion, and reliability-based evidence decomposition. It gives the strongest result across all ADNI metrics and the highest accuracy and macro-F1 on family-disjoint three-class ABCD. Component controls identify multigranular readout and expert diversity as the principal gains, while the incomplete ABCD stratum exposes the benefit of sparse specialization. Strict-removal agreement validates the model's source sensitivity.
