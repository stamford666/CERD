...
def _feature_columns(frame: pd.DataFrame, spec: dict[str, Any], id_column: str) -> list[str]:
    ...
    # ``excluded_feature_columns`` is emitted by the dataset builders as the
    # auditable task-specific denylist (for example, direct ADHD/attention
    # items and algebraically contaminated CBCL scores).  Honour both the
    # legacy generic field and the provenance-oriented field so a later
    # parquet regeneration cannot silently reintroduce a denied predictor.
    excluded = {
        id_column,
        *spec.get("exclude_columns", []),
        *spec.get("excluded_feature_columns", []),
    }
...
def _missingness_before_preprocessing(
    manifest: dict[str, Any], args: Any
) -> bool:
    """Resolve the opt-in preprocessing order without changing legacy manifests.

    A manifest declaration is authoritative so a frozen dataset cannot be
    silently reinterpreted by a launcher flag.  The loader argument exists for
    small programmatic fixtures and future builders; both interfaces default to
    the historical post-preprocessing masking order.
    """

    missingness_spec = manifest.get("missingness")
    if missingness_spec and "apply_before_preprocessing" in missingness_spec:
        value = missingness_spec["apply_before_preprocessing"]
        if not isinstance(value, bool):
            raise ValueError(
                "missingness.apply_before_preprocessing must be boolean"
            )
        return value
    value = getattr(args, "abcd_missingness_before_preprocessing", False)
    if not isinstance(value, (bool, np.bool_)):
        raise ValueError(
            "abcd_missingness_before_preprocessing must be boolean"
        )
    return bool(value)


def _load_missingness_keep_masks(
    ...
    """Load the frozen participant keep mask in canonical label-table order."""

    missingness_spec = manifest.get("missingness")
    if not missingness_spec:
        raise ValueError(
            "ABCD missingness-before-preprocessing requires a missingness table"
        )
    ...
    if missingness.index.has_duplicates or missingness.isna().all(axis=1).any():
        raise ValueError(
            "Missingness table must contain exactly one row for every labeled participant"
        )
    column_map = missingness_spec.get("columns", {})
    keep_masks: dict[str, np.ndarray] = {}
    for name in modality_dict:
        code = specs[name]["code"].upper()
        column = column_map.get(code, code)
        if column not in missingness:
            raise ValueError(
                f"Missingness table has no keep-mask column for modality {code!r}"
            )
        values = missingness[column]
        if values.isna().any():
            raise ValueError(
                f"Missingness keep-mask column {column!r} contains missing values"
            )
        if pd.api.types.is_bool_dtype(values):
            ...
            if not values.isin([0, 1]).all():
                raise ValueError(
                    f"Missingness keep-mask column {column!r} must contain only 0/1"
                )
            keep_mask = values.to_numpy(dtype=bool)
        else:
            ...
            if not normalized_values.isin(["true", "false", "0", "1"]).all():
                raise ValueError(
                    f"Missingness keep-mask column {column!r} is not boolean"
                )
            keep_mask = normalized_values.isin(["true", "1"]).to_numpy(dtype=bool)
        keep_masks[name] = keep_mask
    return keep_masks
...
def validate_abcd_manifest(args: Any) -> dict[str, Any]:
    ...
    if manifest.get("missingness"):
        ...
        if "apply_before_preprocessing" in manifest["missingness"] and not isinstance(
            manifest["missingness"]["apply_before_preprocessing"], bool
        ):
            errors.append(
                "missingness.apply_before_preprocessing must be boolean"
            )
    ...
    source_blacklist = set(
        manifest.get("provenance", {}).get("source_table_blacklist", [])
    )
    included_sources = {
        source
        for item in manifest["modalities"]
        for source in item.get("source_tables", [])
    }
    forbidden_sources = sorted(source_blacklist & included_sources)
    if forbidden_sources:
        raise ValueError(
            "ABCD manifest includes task-blacklisted predictor sources: "
...
def load_abcd_data(args: Any, modality_dict: dict[str, int], patch_embeddings_cls: Any):
    ...
    missingness_before_preprocessing = _missingness_before_preprocessing(
        manifest, args
    )
    preprocessing_keep_masks = (
        _load_missingness_keep_masks(
            manifest, base, ids, id_col, specs, modality_dict
        )
        if missingness_before_preprocessing
        else {}
    )
    ...
    for name, column_idx in modality_dict.items():
        ...
        if missingness_before_preprocessing:
            keep_mask = preprocessing_keep_masks[name]
            available_ids = [
                subject_id
                for row_idx, subject_id in enumerate(ids)
                if subject_id in numeric.index and keep_mask[row_idx]
            ]
            available_id_set = set(available_ids)
            train_subjects = [
                ids[i] for i in train_ids if ids[i] in available_id_set
            ]
        else:
            train_subjects = [
                ids[i] for i in train_ids if ids[i] in numeric.index
            ]
        ...
        if not missingness_before_preprocessing:
            available_ids = [
                subject_id for subject_id in ids if subject_id in numeric.index
            ]
    ...
    missingness_spec = manifest.get("missingness")
    if missingness_spec and not missingness_before_preprocessing:
        missingness = _read_frame(_resolve(base, missingness_spec["path"]))
        if id_col not in missingness:
            raise ValueError(f"Missingness table must contain {id_col!r}")
        missingness[id_col] = missingness[id_col].astype(str)
