#!/usr/bin/env python3
"""Build the frozen ABCD current-status ADHD three-class dataset.

The selected endpoint is derived from baseline parent K-SADS fields:
0 = no recorded ADHD status and <=3 current symptoms in each domain;
1 = no recorded ADHD status and >=4 current symptoms in either domain;
2 = current full ADHD with >=6 current symptoms in either domain.
...
from build_abcd_status3_genenv import (
...
T1_DTI_TABLES = {
    "imaging/mr_y_smri__t1__gm__dsk.parquet": (
        "baseline Desikan cortical T1 gray-matter volume"
    ),
...
def build_current_status_labels(
    root: Path,
    session: str,
    cohort_seed: int,
    control_to_symptomatic_ratio: float,
) -> tuple[pd.DataFrame, dict[str, object]]:
    """Build a cross-sectional control/subthreshold/full-ADHD endpoint.

    Unlike the clinical-course endpoint, the middle class does not mix past
    ADHD and partial remission.  It contains children with elevated current
    symptoms but no recorded current, past, partial-remission, or unspecified
    ADHD diagnosis.
    """
    ...
    valid = values.isin(set(ADHD_VALID_DIAGNOSIS_VALUES)).all(axis=1)
    inattentive = values[list(ADHD_CURRENT_INATTENTIVE_COLUMNS)].eq(1).sum(axis=1)
    hyperactive = values[
    ...
    no_recorded_adhd = valid.copy()
    for column in status_columns:
        no_recorded_adhd &= values[column].ne(1)
    low_pool = no_recorded_adhd & max_domain.le(3)
    symptomatic_no_diagnosis = no_recorded_adhd & max_domain.ge(4)
    current_full = valid & values[present].eq(1) & max_domain.ge(6)

    symptomatic_count = int(symptomatic_no_diagnosis.sum())
    control_count = int(round(symptomatic_count * control_to_symptomatic_ratio))
    control_ids = stable_ids(
        frame.loc[low_pool, "participant_id"].tolist(),
        seed=cohort_seed,
        namespace="adhd_current3_low_control",
        n=control_count,
    )
    if len(control_ids) != control_count:
        raise ValueError("Insufficient low-symptom controls for requested sampling ratio")
    selected_control = frame["participant_id"].isin(control_ids)
    included = selected_control | symptomatic_no_diagnosis | current_full
    target = np.select(
        [selected_control, symptomatic_no_diagnosis, current_full],
        [0, 1, 2],
        default=-1,
    ).astype(np.int64)
    labels = pd.DataFrame(
    ...
    counts = labels["target"].value_counts().sort_index()
    details = {
        "name": "baseline parent K-SADS current ADHD clinical status",
        "session": session,
        "informant": "parent",
        "class_mapping": {
            "0": "no recorded ADHD status and <=3 current symptoms in each domain",
            "1": "no recorded ADHD status and >=4 current symptoms in at least one domain",
            "2": "current full ADHD and >=6 current symptoms in at least one domain",
        },
        "interpretation": {
            "0": "strict low-symptom comparator",
            "1": "elevated/subthreshold current ADHD symptoms without a full diagnosis",
            "2": "current full ADHD",
        },
        "middle_class_excludes": [
            "past full ADHD",
        ...
        "not_administered_code": 555,
        "current_symptom_threshold": 4,
        "full_diagnosis_consistency_threshold": 6,
        "raw_baseline_participants": int(len(frame)),
        "valid_participants": int(valid.sum()),
        "low_symptom_pool": int(low_pool.sum()),
        "sampled_low_symptom_controls": int(selected_control.sum()),
        "symptomatic_without_recorded_adhd": symptomatic_count,
        "symptomatic_4_to_5": int(
            (symptomatic_no_diagnosis & max_domain.between(4, 5)).sum()
        ),
        "symptomatic_6_or_more": int(
            (symptomatic_no_diagnosis & max_domain.ge(6)).sum()
        ),
        "current_full_before_symptom_consistency": int(
            (valid & values[present].eq(1)).sum()
        ),
        "current_full_included": int(current_full.sum()),
        "excluded_past_or_remission": int(
            (
                valid
                & values[present].ne(1)
                & (values[past].eq(1) | values[partial].eq(1))
        ...
        "control_sampling": {
            "seed": cohort_seed,
            "method": "SHA256-stable random sample before family-disjoint splitting",
            "control_to_symptomatic_ratio": control_to_symptomatic_ratio,
        },
        "class_counts": {str(k): int(counts.get(k, 0)) for k in (0, 1, 2)},
        "materialized_label_columns": ["participant_id", "target"],
    }
...
def build_current_spectrum_labels(
    root: Path,
    session: str,
    cohort_seed: int,
    control_to_symptomatic_ratio: float,
) -> tuple[pd.DataFrame, dict[str, object]]:
    """Build low-symptom, symptomatic non-full, and current-full ADHD classes.

    The middle class is defined by *current* symptom burden, not by history:
    it contains every child without current full ADHD who has at least four
    current symptoms in either DSM domain. Past, partial-remission, and
    unspecified status are retained only when this current-symptom criterion
    is met.
    """

    labels, source_details = build_current_status_labels(
        root,
        session,
        cohort_seed,
        control_to_symptomatic_ratio,
    )
    path = root / ADHD_TARGET_TABLE
    frame = pd.read_parquet(path)
    frame = frame[frame["session_id"].astype(str).eq(session)].copy()
    ...
    max_domain = pd.concat([inattentive, hyperactive], axis=1).max(axis=1)
    symptomatic_without_current_full = (
        valid & values[present].ne(1) & max_domain.ge(4)
    )
    all_middle_ids = set(
        frame.loc[symptomatic_without_current_full, "participant_id"]
    )
    existing_middle_ids = set(
        labels.loc[labels["target"].eq(1), "participant_id"]
    )
    added_middle_ids = all_middle_ids - existing_middle_ids
    added = pd.DataFrame(
        {
            "participant_id": sorted(added_middle_ids),
            "target": np.ones(len(added_middle_ids), dtype=np.int64),
        }
    )
    labels = pd.concat([labels, added], ignore_index=True)
    if labels["participant_id"].duplicated().any():
        raise RuntimeError("Duplicate participant after current-spectrum expansion")
    labels = labels.sort_values("participant_id", kind="stable").reset_index(drop=True)
    counts = labels["target"].value_counts().sort_index()

    added_mask = frame["participant_id"].isin(added_middle_ids)
    details = dict(source_details)
    details.pop("middle_class_excludes", None)
    details.update(
        {
            "name": "baseline parent K-SADS current ADHD spectrum status",
            "class_mapping": {
                "0": "no recorded ADHD status and <=3 current symptoms in each domain",
                "1": "no current full ADHD and >=4 current symptoms in at least one domain",
                "2": "current full ADHD and >=6 current symptoms in at least one domain",
            },
            "interpretation": {
                "0": "strict low-symptom comparator",
                "1": "current elevated/subthreshold ADHD-spectrum state without current full ADHD",
                "2": "current full ADHD",
            },
            "middle_class_history_rule": "history alone is insufficient; past, partial-remission, or unspecified status is retained only with >=4 current symptoms in a domain",
            "symptomatic_without_current_full": int(
                symptomatic_without_current_full.sum()
            ),
            "history_positive_symptomatic_added": len(added_middle_ids),
            "history_positive_symptomatic_breakdown": {
                "past_full": int((added_mask & values[past].eq(1)).sum()),
                "partial_remission": int((added_mask & values[partial].eq(1)).sum()),
                "unspecified": int((added_mask & values[unspecified].eq(1)).sum()),
            },
            "class_counts": {
                str(k): int(counts.get(k, 0)) for k in (0, 1, 2)
            },
        }
    )
    return labels, details
...
def build_current_binary_labels(
    ...
    session: str,
    cohort_seed: int,
    control_to_case_ratio: float,
) -> tuple[pd.DataFrame, dict[str, object]]:
    """Build a strict current-ADHD case--control endpoint.

    Controls have no current, past, partial-remission, or unspecified ADHD and
    no more than three current symptoms in either domain. Cases have a current
    full-ADHD diagnosis, at least six current symptoms in one domain, and all
    18 current symptom fields explicitly encoded as 0/1. Historical-only,
    remission-only, unspecified, and symptom-only records are excluded.
    """
    ...
    required = set(status_columns + symptom_columns)
    missing = sorted(required - set(frame.columns))
    if missing:
        raise ValueError(f"Missing ADHD endpoint columns: {missing}")

    status = frame[status_columns].apply(pd.to_numeric, errors="coerce")
    symptoms = frame[symptom_columns].apply(pd.to_numeric, errors="coerce")
    diagnosis_valid = status.isin(set(ADHD_VALID_DIAGNOSIS_VALUES)).all(axis=1)
    symptom_codes_valid = symptoms.isin({0, 1, 888}).all(axis=1)
    explicit_symptoms = symptoms.isin({0, 1}).all(axis=1)
    inattentive = symptoms[list(ADHD_CURRENT_INATTENTIVE_COLUMNS)].eq(1).sum(axis=1)
    ...
    case_count = int(current_case.sum())
    control_count = int(round(case_count * control_to_case_ratio))
    control_ids = stable_ids(
        frame.loc[clean_pool, "participant_id"].tolist(),
        seed=cohort_seed,
        namespace="adhd_current_binary_clean_control",
        n=control_count,
    )
    if len(control_ids) != control_count:
        raise ValueError("Insufficient clean controls for requested sampling ratio")
    selected_control = frame["participant_id"].isin(control_ids)
    included = selected_control | current_case
    target = np.select([selected_control, current_case], [0, 1], default=-1).astype(
        np.int64
    )
    labels = pd.DataFrame(
        {
    ...
    counts = labels["target"].value_counts().sort_index()
    details = {
        "name": "baseline parent K-SADS strict current ADHD case-control status",
        "endpoint_type": "strict low-symptom non-ADHD comparator versus current full ADHD",
        "session": session,
        "informant": "parent",
        "class_mapping": {
            "0": "no current/past/partial-remission/unspecified ADHD and <=3 current symptoms in each domain",
            "1": "current full ADHD with >=6 current symptoms in at least one nine-item domain",
        },
        "available_status_columns": status_columns,
        "current_inattentive_columns": list(ADHD_CURRENT_INATTENTIVE_COLUMNS),
        "current_hyperactive_impulsive_columns": list(
        ...
        "valid_diagnosis_codes": list(ADHD_VALID_DIAGNOSIS_VALUES),
        "case_symptom_codes": [0, 1],
        "not_administered_code": 555,
        "diagnostic_symptom_threshold": 6,
        "raw_baseline_participants": int(len(frame)),
        "clean_control_pool": int(clean_pool.sum()),
        "sampled_clean_controls": int(selected_control.sum()),
        "current_full_before_explicit_symptom_check": int(
            (diagnosis_valid & status[present].eq(1) & max_domain.ge(6)).sum()
        ),
        "current_full_included": case_count,
        "excluded_current_cases_with_nonexplicit_symptoms": int(
            (
                diagnosis_valid
                & status[present].eq(1)
                & max_domain.ge(6)
        ...
        "control_sampling": {
            "seed": cohort_seed,
            "method": "SHA256-stable sampling before family-disjoint splitting",
            "control_to_case_ratio": control_to_case_ratio,
        },
        "class_counts": {str(k): int(counts.get(k, 0)) for k in (0, 1)},
        "materialized_label_columns": ["participant_id", "target"],
    }
    return labels, details
...
def build_presentation_extent_labels(
    root: Path,
    session: str,
    cohort_seed: int,
    control_to_inattentive_ratio: float,
) -> tuple[pd.DataFrame, dict[str, object]]:
    """Group full ADHD by one versus both DSM symptom domains.

    This endpoint preserves exactly the cohort selected by
    :func:`build_presentation_labels`. The only label change is that the
    hyperactive/impulsive-only presentation joins the inattentive-only
    presentation in class 1, while class 2 is restricted to the combined
    presentation. No participant is selected or discarded on the basis of
    predictor values.
    """

    labels, source_details = build_presentation_labels(
        root,
        session,
        cohort_seed,
        control_to_inattentive_ratio,
    )
    ...
    frame = frame[frame["session_id"].astype(str).eq(session)].copy()
    frame["participant_id"] = frame["participant_id"].astype(str)
    frame = frame[frame["participant_id"].isin(set(labels["participant_id"]))]
    frame = frame.set_index("participant_id", drop=False)

    present = "mh_p_ksads__adhd__pres_dx"
    past = "mh_p_ksads__adhd__past_dx"
    status_columns = list(source_details["available_status_columns"])
    present_symptom_columns = [
        *ADHD_CURRENT_INATTENTIVE_COLUMNS,
        *ADHD_CURRENT_HYPERACTIVE_IMPULSIVE_COLUMNS,
    ]
    ...
    selected_symptoms = pd.DataFrame(
        ...
        index=frame.index,
        columns=present_symptom_columns,
    )
    selected_symptoms_valid = selected_symptoms.isin({0, 1}).all(axis=1)
    inattentive_count = selected_symptoms[
        list(ADHD_CURRENT_INATTENTIVE_COLUMNS)
    ].eq(1).sum(axis=1)
    hyperactive_count = selected_symptoms[
        list(ADHD_CURRENT_HYPERACTIVE_IMPULSIVE_COLUMNS)
    ].eq(1).sum(axis=1)
    threshold = ADHD_PRESENTATION_THRESHOLD
    inattentive_positive = inattentive_count.ge(threshold)
    hyperactive_positive = hyperactive_count.ge(threshold)
    single_domain = (
        full_case
        & selected_symptoms_valid
        & (inattentive_positive ^ hyperactive_positive)
    )
    combined = (
        full_case
        & selected_symptoms_valid
        & inattentive_positive
        & hyperactive_positive
    )

    labels = labels.set_index("participant_id", drop=False)
    case_ids = set(labels.loc[labels["target"].ne(0), "participant_id"])
    derived_case_ids = set(frame.loc[single_domain | combined, "participant_id"])
    if case_ids != derived_case_ids:
        raise RuntimeError(
            "Presentation-extent relabeling changed the selected ADHD case cohort"
        )
    single_domain_ids = set(frame.loc[single_domain, "participant_id"])
    combined_ids = set(frame.loc[combined, "participant_id"])
    labels.loc[labels["participant_id"].isin(single_domain_ids), "target"] = 1
    labels.loc[labels["participant_id"].isin(combined_ids), "target"] = 2
    labels = labels.reset_index(drop=True).sort_values(
    ...
    details.update(
        {
            "name": "baseline parent K-SADS ADHD presentation extent",
            "endpoint_type": "low-symptom comparator versus single-domain and combined full-ADHD presentations",
            "class_mapping": {
                "0": "no present/past/partial-remission/unspecified ADHD and <=3 current symptoms in each domain",
                "1": "full ADHD meeting the >=6-symptom threshold in exactly one selected-episode domain (inattentive-only or hyperactive/impulsive-only)",
                "2": "combined full ADHD meeting the >=6-symptom threshold in both selected-episode domains",
            },
            "single_domain_cases": int(single_domain.sum()),
            "combined_cases": int(combined.sum()),
            "class_counts": {
                str(k): int(counts.get(k, 0)) for k in (0, 1, 2)
            },
            "cohort_preservation": {
                "source_endpoint": "presentation3",
                "same_participant_set": True,
                "participants_reassigned_from_class_2_to_class_1": int(
                    source_details["hyperactive_impulsive_only_cases"]
                ),
...
def build_common_presentation_labels(
    root: Path,
    session: str,
    cohort_seed: int,
    control_to_inattentive_ratio: float,
) -> tuple[pd.DataFrame, dict[str, object]]:
    """Retain the two adequately sized, standard ADHD presentations.

    Predominantly hyperactive/impulsive ADHD is kept distinct during endpoint
    construction and then excluded because its available cohort is too small
    to support a stable third diagnostic class.  It is never merged into the
    combined presentation.  The requested sample size can be held fixed by a
    prespecified control sampling ratio.
    """

    presentation, source_details = build_presentation_labels(
        root,
        session,
        cohort_seed,
        control_to_inattentive_ratio,
    )
    extent, _ = build_presentation_extent_labels(
        root,
        session,
        cohort_seed,
        control_to_inattentive_ratio,
    )
    source_target = presentation.set_index("participant_id")["target"]
    extent_target = extent.set_index("participant_id")["target"]
    hyperactive_only_ids = set(
        source_target.index[source_target.eq(2) & extent_target.eq(1)]
    )
    expected_hyperactive_only = int(
        source_details["hyperactive_impulsive_only_cases"]
    )
    if len(hyperactive_only_ids) != expected_hyperactive_only:
        raise RuntimeError(
            "Failed to identify the expected hyperactive/impulsive-only cohort"
        )

    labels = presentation[
        ~presentation["participant_id"].isin(hyperactive_only_ids)
    ].copy()
    labels = labels.sort_values("participant_id", kind="stable").reset_index(drop=True)
    counts = labels["target"].value_counts().sort_index()
    details = dict(source_details)
    details.update(
        {
            "name": "baseline parent K-SADS common ADHD clinical presentations",
            "endpoint_type": "low-symptom comparator versus predominantly inattentive and combined full-ADHD presentations",
            "class_mapping": {
                "0": "no present/past/partial-remission/unspecified ADHD and <=3 current symptoms in each domain",
                "1": "full ADHD with predominantly inattentive presentation in the selected episode",
                "2": "combined full ADHD meeting the >=6-symptom threshold in both selected-episode domains",
            ...
            "excluded_presentation": {
                ...
                "reason": "insufficient cohort size for a stable standalone three-class endpoint",
                "participants": len(hyperactive_only_ids),
                "merged_with_another_class": False,
            },
            "class_counts": {
                str(k): int(counts.get(k, 0)) for k in (0, 1, 2)
            },
...
def main() -> None:
    ...
    parser.add_argument(
        "--control-to-symptomatic-ratio",
        "--control-to-remitted-ratio",
        dest="control_to_remitted_ratio",
        type=float,
        default=1677 / 460,
    ...
    if args.endpoint == "course3":
    ...
    elif args.endpoint == "current3":
        labels, endpoint = build_current_status_labels(
            root,
            args.session,
            args.cohort_seed,
            args.control_to_remitted_ratio,
        )
    elif args.endpoint == "current_spectrum3":
        labels, endpoint = build_current_spectrum_labels(
            root,
            args.session,
            args.cohort_seed,
            args.control_to_remitted_ratio,
    ...
    elif args.endpoint == "presentation_extent3":
        labels, endpoint = build_presentation_extent_labels(
            root,
            args.session,
            args.cohort_seed,
            args.control_to_remitted_ratio,
        )
    elif args.endpoint == "presentation_common3":
        labels, endpoint = build_common_presentation_labels(
            root,
            args.session,
            args.cohort_seed,
            args.control_to_remitted_ratio,
        ...
        labels, endpoint = build_current_binary_labels(
            root,
            args.session,
            args.cohort_seed,
            args.control_to_case_ratio,
        )
    ...
    imaging_receipt.update(
        {
            "definition": "baseline rs-fMRI + SST/n-back/MID task-fMRI + T1 gray-matter volume + DTI FA",
    ...
    manifest = {
        ...
        "label": {
            "path": "labels.parquet",
            "column": "target",
            "class_values": sorted(int(value) for value in labels["target"].unique()),
        },
        "splits": "splits.json",
        ...
        "provenance": {
            ...
            "modality_grouping": {
                "I": "rs-fMRI + SST/n-back/MID task-fMRI + T1 gray-matter volume + DTI FA",
