...
manifest=${DATASET_MANIFEST:-"$workspace/data/abcd_adhd_severity3_snp_v1_n3000_random_missing15/manifest.json"}
root=${CAMPAIGN_ROOT:-"$here/abcd_severity3_n3000_missing15_full_rerun_v2"}
...
payload = {
    ...
    "training_protocol": {
        ...
        "cerd_sparse_routing": {"num_experts": 4, "top_k": 2},
    },
...
run_validation() {
  ...
  case "$model" in
    our_moe)
      ...
      patches=8; experts=4; topk=2; modality_dropout=0.25
      ;;
    flex_moe)
...
summary = {
    "schema": "abcd-severity3-n3000-missing15-e4k2-formal-v2",
