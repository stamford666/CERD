...
manifest="$workspace/data/abcd_adhd_presentation3_snp_v4_random_missing15/manifest.json"
root=${CAMPAIGN_ROOT:-"$here/abcd_presentation3_component_ablation_v1"}

arms=(dense_backbone no_multigranular_decomposition joint_branch_only no_completion no_provenance uniform_branch_weights)
seeds=(31 32 33)
...
run_validation() {
  ...
  local output="$root/$arm/validation"
  local log="$root/logs/$arm/validation_seed${seed}.log"
  local existing
  ...
  "$python_bin" -u "$runner" \
    ...
    --class_weight_power 1 --sampler_power 0 \
...
run_formal() {
  local gpu=$1 arm=$2 seed=$3
  local source output log existing
  ...
  existing="$output/abcd/our_moe_seed${seed}.json"
  if [[ -f "$existing" ]] && rg -q '"status": "complete"' "$existing"; then
    echo "[skip formal] $existing"
    return 0
  fi
  mkdir -p "$output" "$(dirname "$log")"
  CUDA_VISIBLE_DEVICES="$gpu" \
...
if [[ "$status" -ne 0 ]]; then
  echo "At least one ablation worker failed; inspect $root/logs." >&2
  exit "$status"
fi

"$python_bin" "$here/summarize_abcd_presentation3_component_ablation_v1.py" --campaign-root "$root"
