...
manifest=${DATASET_MANIFEST:-"$workspace/data/abcd_adhd_presentation3_snp_v4_random_missing15/manifest.json"}
output_dir=${OUTPUT_ROOT:-"$here/abcd_adhd_presentation3_snp_missing15_v4_validation"}
...
exec "$python_bin" "$here/baseline_runner.py" \
    ...
    --early_stopping_patience 10 --early_stopping_min_epochs 0 \
    ...
    --batch_size 64 --hidden_dim 128 --num_patches 8 \
    --num_heads 4 --num_layers_fus 1 --num_layers_pred 1 \
    --num_experts 16 --num_routers 1 --top_k 4 --dropout 0.35 \
